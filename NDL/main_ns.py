...
def pickASentence(languageDomain,infl_list,dist):
    ...
    ch = random.choice([choice(list1), choice(list2)], p=[1-dist, dist])
    #print(list2)
...
def childLearnsLanguage(ndr, languageDomain,language,numberofsentences,infl_list,sl,c,growth):
	...
	if j<3566210:
            s = pickASentence(languageDomain,infl_list,0.16)
	else:
	    s = pickASentence(languageDomain,infl_list,0.18)
...
if __name__ == '__main__':
    ...
    #n_list=[[],[],[],[]]
    #n_max_list=[-1]*numberofsentences
    #n_min_list=[50000]*numberofsentences
    ct=0
    f='0'
    if growth==1:
	f=open("output_ns_linear_avg.csv", "wb")
    else:
	f=open("output_ns_e_avg.csv", "wb")
    n_list=[]
    for result in results:
	learn_res=result[0][0][0:numberofsentences]
	n_list.append(learn_res)
	#writer = csv.writer(f)
        #writer.writerow(learn_res)
    	#n_list[0].append(result[0][0][4566082])
	#n_list[1].append(result[0][0][5610392])
	#n_list[2].append(result[0][0][6688032])
	#n_list[3].append(result[0][0][7787891])
        #ns_list+=np.array(result[0][0][0:numberofsentences])
	#x=np.arange(0,numberofsentences)
        #y=np.array(learn_res)
	#plt.plot(x,y)

	#for i in range(0,numberofsentences):
	    #if n_max_list[i]<learn_res[i]:
		#n_max_list[i]=learn_res[i]
	    #if n_min_list[i]>learn_res[i]:
		#n_min_list[i]=learn_res[i]
    ...
    #y=ns_list/100
    #print("means are:",y[4566082],y[5610392],y[6688032],y[7787891])
    #y_max=np.array(n_max_list)
    #y_min=np.array(n_min_list)
    #std_list=[]
    #for ns in n_list:
	#std_list.append(statistics.stdev(ns))
    #print("std_dev is:",std_list)
    #plt.xlabel("No. of utterances")
    #plt.ylabel("NS parameter confidence")
    ...
    plt.axvline(x=5610392,linestyle='dashed',label='age=3',c='c')
    plt.axvline(x=6688032,linestyle='dashed',label='age=3;6',c='m')
    plt.axvline(x=7787891,linestyle='dashed',label='age=4',c='y')
    plt.legend()
    #4566082 5610392 6688032 7787891
    #plt.xlim([150000,500000])
    if growth==1:
    	plt.savefig('NDL_ns_linear.png')
    else:
        plt.savefig('NDL_ns_e.png')
    '''
